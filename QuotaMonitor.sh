...
Setup() {
    ...
    iptables -D FORWARD -i br0      -j $TABLE_OUT 2>/dev/null       # v1.05
    iptables -D FORWARD -i $WAN_IF -j $TABLE_IN 2>/dev/null         # v1.05
                ...
                do
                    ...
                    for HOSTNAME in $MONITORED_DEVICES
                        do
                            ...
                            USED=0      # v1.05
                            [ -n "$RECV" ] && USED=$(expr "$RECV" + "$USED")                        # v1.05
                            [ -n "$XMIT" ] && USED=$(expr "$XMIT" + "$USED")                        # v1.05
                            ...
                            if [ $DELTA_RECV = 0 ] && [ $DELTA_XMIT = 0 ];then          # v1.06
                                COLOUR_USED=$cBGRA                                          # v1.06 Make it easier to see no change
                            fi
...
modprobe -sv xt_comment.ko                                  # v1.05
